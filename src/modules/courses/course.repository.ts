...
export class CourseRepository {
  ...
  async teacherIsValid(id: number): Promise<boolean> {
    const [rows] = await pool.query<RowDataPacket[]>(
      `SELECT 1
       FROM usuarios
       WHERE id = ?
  ...
  async findCourseOwner(courseId: number): Promise<{
    id: number;
    docente_id: number;
    estado: CourseStatus;
    tipo_acceso: CourseAccessType;
    ...
    >(
      `SELECT id, docente_id, estado, tipo_acceso, precio, slug, fecha_publicacion
       FROM cursos
       WHERE id = ?
       LIMIT 1`,
      [courseId]
  ...
  async slugExists(slug: string, excludeId?: number): Promise<boolean> {
    ...
    const [rows] = await pool.query<RowDataPacket[]>(
      `SELECT 1 FROM cursos WHERE slug = ? ${excludeSql} LIMIT 1`,
      params
    );
    return rows.length > 0;
  }
  ...
  async listPublished(
    ...
    const where: string[] = ["c.estado = 'publicado'", "cat.estado = 'activo'"];
  ...
  async findPublishedDetailById(id: number): Promise<CourseDetail | null> {
  ...
  async findDetailById(id: number): Promise<CourseDetailPrivate | null> {
    const [rows] = await pool.query<CourseDetailRow[]>(
      `SELECT
        c.id, c.titulo, c.slug, c.descripcion_corta, c.descripcion, c.imagen_url, c.video_intro_url,
        c.tipo_acceso, c.precio, c.nivel, c.estado, c.duracion_horas, c.requisitos, c.objetivos,
  ...
  async hideCourseById(id: number): Promise<boolean> {
    const [result] = await pool.execute<ResultSetHeader>(
      `UPDATE cursos
  ...
  async listTeaching(
    ...
    pagination: Pagination,
    search?: string
  ): Promise<{ items: CourseListItemTeaching[]; total: number }> {
    const where: string[] = [];
    const params: Array<string | number> = [];
  ...
  async listEnrolled(userId: number): Promise<EnrolledCourseItem[]> {
    const [rows] = await pool.query<EnrolledRow[]>(
      `SELECT
        c.id, c.titulo, c.slug, c.descripcion_corta, c.imagen_url, c.tipo_acceso, c.precio, c.nivel,
        c.estado, c.fecha_publicacion, c.created_at,
        cat.id as categoria_id, cat.nombre as categoria_nombre, cat.imagen_url as categoria_imagen_url,
...
function mapCourseListRow(r: CourseListRow): CourseListItem {
  return {
    id: r.id,
    titulo: r.titulo,
    slug: r.slug,
...
function mapCourseDetailRow(r: CourseDetailRow): CourseDetail {
  return {
    ...mapCourseListRow(r),
...
function mapCourseDetailPrivateRow(r: CourseDetailRow): CourseDetailPrivate {
  return {
