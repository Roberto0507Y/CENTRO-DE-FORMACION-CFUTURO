...
export const createQuizBodySchema = z
  .object({
    ...
    tipo: z.enum(["regular", "admision"]).optional(),
    puntaje_total: numberFromBody("puntaje_total").pipe(z.number().min(0.01).max(100000)).optional(),
    porcentaje_aprobacion: numberFromBody("porcentaje_aprobacion").pipe(z.number().min(1).max(100)).optional(),
    precio_admision: numberFromBody("precio_admision").pipe(z.number().min(0).max(100000)).optional(),
    payment_link_admision: z.string().url().max(500).nullable().optional(),
    ...
    requiere_pago_reintento: booleanFromBody().optional(),
