...
const gradebook_repository_1 = require("./gradebook.repository");
function roundPercent(value) {
    return Math.round(value * 100) / 100;
}
function buildSummary(tareas, asistencia) {
    ...
    for (const tarea of tareas) {
        ...
        if (!Number.isFinite(calificacion) || !Number.isFinite(puntos) || puntos <= 0)
            continue;
        tareasCalificadas += 1;
...
class GradebookService {
    constructor() {
        this.repo = new gradebook_repository_1.GradebookRepository();
    }
    async getMyCourseGradebook(requester, courseId) {
        ...
        if (curso.estado !== "publicado")
            throw (0, httpErrors_1.notFound)("Curso no encontrado");
        if (curso.inscripcion_estado !== "activa")
