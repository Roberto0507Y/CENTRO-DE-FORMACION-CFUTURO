...
class AuthService {
    ...
    async register(input) {
        ...
        if (existing)
            throw (0, httpErrors_1.conflict)("El correo ya está registrado");
...
<html lang="es">
  ...
  <body style="margin:0;padding:0;background:#f1f5f9;">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#f1f5f9;">
      ...
      `.trim();
            await (0, mailer_service_1.sendMail)({ to: user.correo, subject, text, html });
        }
        catch (err) {
    ...
    async resetPassword(input) {
        ...
        if (!preRow)
            throw (0, httpErrors_1.badRequest)("Token inválido o expirado");
        if (preRow.used_at)
            throw (0, httpErrors_1.badRequest)("Token ya fue utilizado");
