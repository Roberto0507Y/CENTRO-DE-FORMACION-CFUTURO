...
class AdminRepository {
    async metrics() {
        const [userRows] = await db_1.pool.query(`SELECT
        COUNT(*) AS total,
      ...
      FROM usuarios`);
        const [courseRows] = await db_1.pool.query(`SELECT
        COUNT(*) AS total,
        SUM(estado = 'publicado') AS published,
        SUM(estado = 'borrador') AS draft,
        SUM(estado = 'oculto') AS hidden
      FROM cursos`);
        const [paymentRows] = await db_1.pool.query(`SELECT
        COUNT(*) AS total,
      ...
      FROM pagos`);
        const [enrollmentRows] = await db_1.pool.query(`SELECT
        COUNT(*) AS total,
      ...
      FROM inscripciones`);
        const [signupRows] = await db_1.pool.query(`SELECT id, nombres, apellidos, correo, rol, estado, created_at
       FROM usuarios
       ORDER BY created_at DESC
       LIMIT 5`);
        const [loginRows] = await db_1.pool.query(`SELECT id, nombres, apellidos, correo, rol, estado, ultimo_login
       FROM usuarios
       WHERE ultimo_login IS NOT NULL
       ORDER BY ultimo_login DESC
       LIMIT 5`);
        const [recentPaymentRows] = await db_1.pool.query(`SELECT
        p.id,
        p.usuario_id,
        u.nombres,
        u.apellidos,
